 ...
 * @access  Public
 */
router.get('/category/:category', validatePagination, asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    sendPaginatedResponse(res, productData, total, page, limit, `Products in category ${category} retrieved successfully`);
    return;
  } catch (error) {
